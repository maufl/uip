...
impl UnixState {
    pub fn new(ctl_socket: String, upstream: Sender<(Identifier, u16, u16, Bytes)>) -> UnixState {
        UnixState {
...
impl Shared<UnixState> {
    ...
    async fn open_ctl_socket(&self) -> Result<(), tokio::io::Error> {
        if self.read().ctl_socket == "" {
            ...
            return Err(tokio::io::Error::new(
                ...
                "No unix control socket path is given",
            ));
        }
        let listener = UnixListener::bind(&self.read().ctl_socket)?;
        loop {
    ...
    pub async fn handle_new_stream(&self, connection: UnixStream) -> Result<(), Error> {
        ...
        match read_half.next().await {
            ...
            Some(Ok(Frame::Data(_, _, _))) => {
                warn!("Received data on an unix connection that was not bound")
            }
            Some(Ok(Frame::Error(err))) => warn!(
                "Received an error on an unix connection tat was not bound: {:?}",
                err
            ),
    ...
    fn spawn_read_task(
        ...
        tokio::spawn(async move {
            loop {
                match read_half.next().await {
                    Some(Ok(Frame::Data(host_id, dst_port, data))) => {
                        state
                            .send_frame(host_id, src_port, dst_port, data.into())
                            .await
                    }
    ...
    fn spawn_write_task(
        &self,
        src_port: u16,
        write_half: FramedWrite<tokio::io::WriteHalf<tokio::net::UnixStream>, ControlProtocolCodec>,
    ) {
        let (sender, receiver) = channel::<Frame>(10);
        self.write().connections.insert(src_port, sender);
        tokio::spawn(async move {
            if let Err(err) = ReceiverStream::new(receiver)
                .map(|f| Ok(f))
                .forward(write_half)
                .await
            {
                warn!(
                    "Forwarding data frame to local Unix connection failed: {}",
                    err
                );
            }
        });
    ...
    pub async fn send_frame(&self, host_id: Identifier, src_port: u16, dst_port: u16, data: Bytes) {
        let sender = self.write().upstream.clone();
        let res = sender.send((host_id, src_port, dst_port, data)).await;
        if let Err(err) = res {
    ...
    pub async fn deliver_frame(
        ...
        let socket = if let Some(socket) = self.read().connections.get(&local_port).cloned() {
            socket
        } else {
            return;
        };
